...
void main() {
  group('dispatch()', () {
    test('single event updates state', () {
    ...
    test('multiple sequential dispatches accumulate', () {
      final container = createContainer();
    ...
    test('SetCount overrides current state', () {
      final container = createContainer();
      final notifier = container.read(counterProvider.notifier);

      notifier.dispatch(Increment());
      notifier.dispatch(Increment());
      notifier.dispatch(SetCount(100));
      expect(container.read(counterProvider), 100);
    });

    test('skips update when reduce returns identical state', () {
      final container = createContainer();
      ...
      notifier.dispatch(Reset());
      // For int primitives, identical(0, 0) is true
      expect(notifyCount, 0);
    });

    test('does not skip update when values are equal but not identical', () {
      final container = createContainer();
